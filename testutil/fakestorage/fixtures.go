package fakestorage // import "go.khulnasoft.com/testutil/fakestorage"
...
func ensureHTTPServerImage(t testing.TB) {
	...
	if _, err = os.Stat("../contrib/httpserver/httpserver"); os.IsNotExist(err) {
		...
		cmd := exec.Command(goCmd, "build", "-o", filepath.Join(tmp, "httpserver"), "go.khulnasoft.com/contrib/httpserver")

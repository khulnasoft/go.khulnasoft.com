...
Function Get-UpstreamCommit() {
    Invoke-Expression "git fetch -q https://go.khulnasoft.com.git refs/heads/master"
...
Function Validate-DCO($headCommit, $upstreamCommit) {
    ...
    if ($badCommits.Length -eq 0) {
        ...
        $e += "https://go.khulnasoft.com/blob/master/CONTRIBUTING.md#sign-your-work`n"
        Throw $e
    }
}
...
Function Validate-PkgImports($headCommit, $upstreamCommit) {
    ...
    $badFiles=@(); $files | ForEach-Object{
        ...
        $imports = @() + $imports -NotMatch "^go.khulnasoft.com/pkg/" `
                                  -NotMatch "^go.khulnasoft.com/vendor" `
                                  -NotMatch "^go.khulnasoft.com/internal" `
                                  -Match "^go.khulnasoft.com" `
                                  -Replace "`n", ""
...
Function Run-UnitTests() {
    ...
    $testPath="./..."
    $goListCommand = "go list -e -f '{{if ne .Name """ + '\"go.khulnasoft.com\"' + """}}{{.ImportPath}}{{end}}' $testPath"
    $pkgList = $(Invoke-Expression $goListCommand)
    if ($LASTEXITCODE -ne 0) { Throw "go list for unit tests failed" }
    $pkgList = $pkgList | Select-String -Pattern "go.khulnasoft.com"
    $pkgList = $pkgList | Select-String -NotMatch "go.khulnasoft.com/vendor"
    $pkgList = $pkgList | Select-String -NotMatch "go.khulnasoft.com/man"
    $pkgList = $pkgList | Select-String -NotMatch "go.khulnasoft.com/integration"
...
Try {
    ...
    $ldflags = "-X 'go.khulnasoft.com/dockerversion.Version="+$dockerVersion+"'"
    $ldflags += " -X 'go.khulnasoft.com/dockerversion.GitCommit="+$gitCommit+"'"
    $ldflags += " -X 'go.khulnasoft.com/dockerversion.BuildTime="+$env:BUILDTIME+"'"
    $ldflags += " -X 'go.khulnasoft.com/dockerversion.PlatformName="+$env:PLATFORM+"'"
    $ldflags += " -X 'go.khulnasoft.com/dockerversion.ProductName="+$env:PRODUCT+"'"
    $ldflags += " -X 'go.khulnasoft.com/dockerversion.DefaultProductLicense="+$env:DEFAULT_PRODUCT_LICENSE+"'"

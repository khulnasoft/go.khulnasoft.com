...
WORKDIR /go/src/github.com/khulnasoft/distribution

# REGISTRY_VERSION_SCHEMA1 specifies the version of the registry to build and
# install from the https://github.com/khulnasoft/distribution repository. This is
...
RUN --mount=from=registry-src,src=/usr/src/registry,rw \
  ...
  set -ex
  export GOPATH="/go/src/github.com/khulnasoft/distribution/Godeps/_workspace:$GOPATH"
...
WORKDIR /go/src/go.khulnasoft.com
...
RUN ln -sfv /go/src/go.khulnasoft.com/.bashrc ~/.bashrc
...
RUN git config --global --add safe.directory $GOPATH/src/go.khulnasoft.com

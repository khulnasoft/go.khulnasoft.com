...
var (
	// During a release, this will be set to the release tag, e.g. "khulnasoft/v4.5.7"
...
type Provenance struct {
	// Version of the khulnasoft binary.
...
func GetProvenance() Provenance {
	...
	for _, dep := range info.Deps {
		if dep != nil && dep.Path == "go.khulnasoft.com/khulnasoft/v5" {
...
// Semver returns the semantic version of khulnasoft.
// khulnasoft version is set in format "khulnasoft/vX.X.X" in every release.
// X.X.X is a semver. If the version string is not in this format,
// return the original version string
func (v Provenance) Semver() string {
	return strings.TrimPrefix(v.Version, "khulnasoft/")
}

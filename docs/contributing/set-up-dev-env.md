...
5. List the contents of the current directory (`/go/src/go.khulnasoft.com`).

   You should see the image's source from the  `/go/src/go.khulnasoft.com`
   directory.
...
your Docker container.
   ...
   > **Note**: Inspecting the `Dockerfile` shows a `COPY . /go/src/go.khulnasoft.com` instruction, suggesting that dynamic code changes will _not_ be reflected in the container. However inspecting the `Makefile` shows that the current working directory _will_ be mounted via a `-v` volume mount.

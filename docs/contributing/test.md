...
2.  Start a Moby development image.
    ...
    ```bash
    $ docker run --privileged --rm -ti -v `pwd`:/go/src/go.khulnasoft.com docker-dev:dry-run-test /bin/bash
    ```
...
```bash
$ TESTDIRS='go.khulnasoft.com/opts' make test-unit
```
...
```bash
$ TESTDIRS='go.khulnasoft.com/opts' TESTFLAGS='-test.run ^TestValidateIPAddress$' make test-unit
```
...
2.  Change to the `moby` source directory.
    ...
    ```bash
    $ cd /c/gopath/src/go.khulnasoft.com
    ```
